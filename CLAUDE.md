...
# Run the CLI
go run ./cmd/redmap run --org "Acme Corp" --domain acme.com
go run ./cmd/redmap list
```
...
RedMap uses a self-registering plugin architecture. All plugins implement `pkg/plugins.Plugin` and register themselves via `init()` functions:
